# -*- coding: utf-8 -*-
"""
Created on Wed Jun 24 22:42:30 2020
...
class Window(QDialog):
    ...
    def getData(self):
        ...
        #First thing we'll need to do is connect to the database
        con = sqlite3.connect('arkham.db')
        c = con.cursor()
        #Now that we are connected to the database, we want to generate a new unique ID for our new monster
        c.execute('SELECT MAX(id) FROM monsters')
        unique_id = c.fetchone()[0]+1
        
        #Now, we just need the rest of the attributes
        monsterAttributes = {}
        name = self.name.text()
        ...
        # First things first: before inserting a new monster, we need to generate it's unique ID
        sql_command = "INSERT INTO monsters VALUES ({0}, {1}, {2}, symbol)"
        print(sql_command.format(unique_id, name, movment))
